...
int main(int argc, char *argv[]) {
  ...
  ranging_pkt.packet_number = 0;
  ranging_pkt.RoundA = 0;
  ranging_pkt.DelayA = 0;
  ranging_pkt.RoundB = 0;
  ranging_pkt.DelayB = 0;
  ranging_pkt.power = 0.0;

  uint8_t tx_buf[ENCODED_RANGING_PKT_LENGTH + 1];
  tx_buf[0] = RANGING_MSG_TYPE;

  uint8_t empty_buf[ENCODED_RANGING_PKT_LENGTH + 1];
  empty_buf[0] = RANGING_MSG_TYPE;

  uint8_t rx_buf[MAX_PACKET_LEN + 1];

  encode_ranging_pkt(&ranging_pkt, &empty_buf[1]);

  uint64_t tx_timestamp = 0;
  uint64_t rx_timestamp = 0;

  ret = sendto_ts(sd, empty_buf, ENCODED_RANGING_PKT_LENGTH + 1, 0,
  ...
  while (1) {
    ...
    if (rx_buf[0] != RANGING_MSG_TYPE) {
      // printf("Received message is not ranging message\n");
      continue;
    }

    rx_timestamp = rx_ts_tmp;
    ...
    if (ranging_pkt.packet_number == 3) {
      ...
      long double tof = ToF_DS(&ranging_pkt);
